...
var avg_blue = function (data) {
  ...
  var only_blue = data.filter(function(s) {
    return (s.source !== 'oficial' && s.source !== 'la_nacion' && !isNaN(s.value_sell) && !isNaN(s.value_buy) && s.value_sell > 0 && s.value_buy > 0);
  });

  var oficial = _.find(data, {source: 'oficial'});
  ...
  var lastDate = function (a, b) {
    return new Date(Math.max(a, new Date(b.date)));
  };
  ...
  return {
    ...
    oficial: {
      sell: parseFloat(oficial.value_sell.toFixed(4)),
      buy: parseFloat(oficial.value_buy.toFixed(4)),
    },
